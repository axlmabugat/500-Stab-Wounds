        ...
        <div class="col">
            <div class="neo-card p-4">
                <div class="card-body">
                    ...
                    <p class="neo-text">{{ number_format($totalSales, 2) }}</p>
                </div>
            </div>
        </div>
        <div class="col">
            <div class="neo-card p-4">
                <div class="card-body">
                    ...
                    <p class="neo-text">{{ $overallSalesCount }}</p>
                </div>
            </div>
        </div>
        <div class="col">
            <div class="neo-card p-4">
                <div class="card-body">
                    ...
                    <ul>
                        @foreach($productCountPerRegion as $region => $count)
                            <li>{{ $region }}: {{ $count }}</li>
                        @endforeach
                    </ul>
                </div>
...
<script>
    new Chart(document.getElementById('lineChart').getContext('2d'), {
        type: 'line',
        data: {
            labels: {!! json_encode(array_keys($productsSoldPerMonth->toArray())) !!},
            datasets: [{
                label: 'Products Sold per Month',
                data: {!! json_encode(array_values($productsSoldPerMonth->toArray())) !!},
                borderColor: '#00adb5',
    ...
    new Chart(document.getElementById('barChart').getContext('2d'), {
        type: 'bar',
        data: {
            labels: {!! json_encode(array_keys($productsSoldPerRegion->toArray())) !!},
            datasets: [{
                label: 'Products Sold per Region',
                data: {!! json_encode(array_values($productsSoldPerRegion->toArray())) !!},
